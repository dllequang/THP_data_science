...
puts "Le pseudo le plus cours est #{journalists.min { |a, b| a.length <=> b.length }}."

# Combien y-a-t'il de handle contenant 5 caractères (le @ ne compte pas pour un caractère)

journalists_caracters = []
journalists.each do |journalists|
a = journalists.size
journalists_caracters << a
end
puts "Il y a #{journalists_caracters.count(6)} pseudo comptant 5 caractères."

# Combien commencent par une majuscule (première lettre juste après le @) ?
arobase_out = []
arobase_out = journalists.select do |counting| counting.match(/^@[A-Z]/)
end
puts "Il y a #{arobase_out.length} pseudos commençant par une majuscule."

# Trie la liste de handle par ordre alphabétique.
puts "La liste par ordre alphabétique (A --> Z): "
journalists.map!(&:upcase)
...
print journalists.min(381) { |a, b| a.length <=> b.length }

#Quelle est la position dans l'array de la personne @epenser ?
puts "Il est où @epenser dans tout ça?"
puts "Regarde le n° #{journalists.index("@EPENSER")}."
puts "Je vais m'arrêter là, ça surchauffe un peut trop pour moi là."
